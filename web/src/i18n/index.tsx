...
const en: Record<string, string> = {
  ...
  'network.removeMemberConfirm': 'Are you sure you want to remove member "{{name}}" from the network? This removes the member record from the current network.',
  'password.changedWithRevoked': 'Password updated successfully and removed {{count}} other sessions',
  'sessions.otherRemovedWithCount': 'Removed {{count}} other sessions',
  'users.createSuccess': 'Created user {{name}}. Share the temporary password through another channel right away.',
  'users.resetPasswordSuccess': 'Generated a new temporary password for {{name}} and revoked {{count}} existing sessions.',
  'users.deleteSuccess': 'Deleted user {{name}} and transferred {{count}} networks.',
...
const zh: Record<string, string> = {
  ...
  'network.removeMemberConfirm': '确定要将成员“{{name}}”从网络中移除吗？此操作会删除该成员在当前网络中的记录。',
  'password.changedWithRevoked': '密码修改成功，并已移除其他会话 {{count}} 个',
  'sessions.otherRemovedWithCount': '已移除其他会话 {{count}} 个',
  'users.createSuccess': '已创建用户 {{name}}，请立即通过其他方式告知其临时密码',
  'users.resetPasswordSuccess': '已为 {{name}} 生成新的临时密码，并吊销 {{count}} 个现有会话',
  'users.deleteSuccess': '已删除用户 {{name}}，并转移 {{count}} 个网络',
...
const messageCodes: Record<string, { en: string; 'zh-CN': string }> = {
  ...
  'setup.unsupported_database': { en: 'Only SQLite is currently supported', 'zh-CN': '当前仅支持 SQLite' },
  'setup.invalid_config': { en: 'Setup configuration is incomplete', 'zh-CN': '设置配置不完整' },
  'setup.database_connection_failed': { en: 'Database connection failed', 'zh-CN': '数据库连接失败' },
  'setup.database_initialization_failed': { en: 'Database initialization failed', 'zh-CN': '数据库初始化失败' },
  'setup.database_config_save_failed': { en: 'Failed to save database configuration', 'zh-CN': '保存数据库配置失败' },
  'setup.config_save_failed': { en: 'Failed to save setup configuration', 'zh-CN': '保存设置配置失败' },
  'setup.zerotier_config_save_failed': { en: 'Failed to save ZeroTier configuration', 'zh-CN': '保存 ZeroTier 配置失败' },
  'setup.zerotier_client_create_failed': { en: 'Failed to create ZeroTier client', 'zh-CN': '创建 ZeroTier 客户端失败' },
  'setup.zerotier_validation_failed': { en: 'ZeroTier controller validation failed', 'zh-CN': 'ZeroTier 控制器验证失败' },
  'setup.already_initialized': { en: 'System is already initialized', 'zh-CN': '系统已初始化' },
  'setup.admin_state_check_failed': { en: 'Failed to confirm administrator state', 'zh-CN': '确认管理员状态失败' },
  'setup.admin_creation_init_failed': { en: 'Failed to initialize administrator account creation', 'zh-CN': '初始化管理员账户创建失败' },
  'setup.database_reopen_failed': { en: 'Failed to reopen configured database', 'zh-CN': '重新打开已配置数据库失败' },
  'setup.secret_generation_failed': { en: 'Failed to generate secure secret', 'zh-CN': '生成安全密钥失败' },
  'setup.initialization_state_failed': { en: 'Failed to update initialization state', 'zh-CN': '更新初始化状态失败' },
  'setup.admin_required': { en: 'create the first administrator account first', 'zh-CN': '请先创建第一个管理员账户' },
  'setup.zerotier_unavailable': { en: 'ZeroTier controller is currently unavailable', 'zh-CN': 'ZeroTier 控制器当前不可用' },
}
...
const rawEn: Record<string, string> = {
  ...
  '确定要将成员': 'Are you sure you want to remove member',
  '从网络中移除吗？此操作会删除该成员在当前网络中的记录。': 'from the network? This removes the member record from the current network.',
  '编辑网络成员': 'Edit network member',
  '无': 'None',
  '托管 IP': 'Managed IPs',
  '高级设置': 'Advanced Settings',
  '默认会以一个 IPv4 子网作为网络边界。自动分配地址池必须完全落在该子网内。': 'An IPv4 subnet is used as the network boundary by default. Auto-assignment pools must stay fully within that subnet.',
  'IPv4 子网': 'IPv4 subnet',
  '例如 10.22.2.0/24': 'e.g. 10.22.2.0/24',
  '生成新的子网与范围': 'Generate new subnet and range',
  '保存时会自动将该子网同步为网络的主托管路由。': 'Saving automatically syncs this subnet as the network primary managed route.',
  '自动分配 IPv4 地址给新成员设备': 'Auto-assign IPv4 addresses to new member devices',
  '关闭后会收起自动分配范围，并在保存时清空 `ipAssignmentPools`。成员手动指定 IP 不受影响。': 'When disabled, the auto-assignment range is collapsed and `ipAssignmentPools` is cleared on save. Manually assigned member IPs are not affected.',
  '起始 IPv4': 'Start IPv4',
  '结束 IPv4': 'End IPv4',
  '例如 10.22.2.1': 'e.g. 10.22.2.1',
  '例如 10.22.2.254': 'e.g. 10.22.2.254',
  '必须落在 ': 'Must be inside ',
  ' 内': '',
  '请先填写 IPv4 子网': 'Enter the IPv4 subnet first',
  '当前 IPv4 自动分配范围': 'Active IPv4 Auto-Assign Range',
  '使用 `Set Range` 将当前输入加入自动分配地址池列表。': 'Use `Set Range` to add the current values to the auto-assignment pool list.',
  '设置范围': 'Set Range',
  '起始': 'Start',
  '结束': 'End',
  '尚未配置自动分配地址池。': 'No auto-assignment address pools configured yet.',
  'IPv6 默认不分配。只有自定义 IPv6 范围依赖手动填写子网；RFC4193 和 6PLANE 由控制器自动派生。': 'IPv6 is not assigned by default. Only custom IPv6 ranges depend on a manually entered subnet; RFC4193 and 6PLANE are derived by the controller.',
  'IPv6 子网': 'IPv6 subnet',
  '例如 fd00::/48': 'e.g. fd00::/48',
  '从自定义 IPv6 范围分配': 'Assign from Custom IPv6 Range',
  '需要先填写合法的 IPv6 子网，然后才能配置自定义 IPv6 地址池。': 'Enter a valid IPv6 subnet before configuring custom IPv6 address pools.',
  '起始 IPv6': 'Start IPv6',
  '结束 IPv6': 'End IPv6',
  '例如 fd00::1000': 'e.g. fd00::1000',
  '例如 fd00::1fff': 'e.g. fd00::1fff',
  '请先填写 IPv6 子网': 'Enter the IPv6 subnet first',
  '当前 IPv6 自动分配范围': 'Active IPv6 Auto-Assign Range',
  '使用 `Set Range` 将当前输入加入 IPv6 地址池列表。': 'Use `Set Range` to add the current values to the IPv6 address pool list.',
  '尚未配置 IPv6 地址池。': 'No IPv6 address pools configured yet.',
  '分配 RFC4193 唯一本地地址（每台设备 /128）': 'Assign RFC4193 Unique Local Addresses (/128 per device)',
  '自动为每个设备分配稳定的唯一本地 IPv6 地址。无需额外配置。': 'Automatically assign a stable unique local IPv6 address to each device. No extra configuration required.',
  '分配 6PLANE 路由地址（每台设备 /80）': 'Assign 6PLANE Routed Addresses (/80 per device)',
  '为每个成员分配由节点 ID 派生的可路由 IPv6 地址。无需额外配置。': 'Assign each member a routed IPv6 address derived from its node ID. No extra configuration required.',
  '为网络内的自定义域名解析配置 DNS。每个网络只允许一个搜索域，但可以配置多个 DNS 服务器。': 'Configure DNS for custom domain resolution inside the network. Each network allows one search domain and multiple DNS servers.',
  '搜索域': 'Search Domain',
  '例如 home.arpa': 'e.g. home.arpa',
  'DNS 服务器': 'DNS Server',
  '例如 1.1.1.1 或 fd00::53': 'e.g. 1.1.1.1 or fd00::53',
  '添加 DNS': 'Add DNS',
  '服务器': 'Server',
  '尚未配置 DNS 服务器。': 'No DNS servers configured yet.',
  '定义该网络还能到达的附加网段。主 IPv4/IPv6 子网由上面的分配区块维护，这里只管理额外的托管路由。': 'Define additional networks this network can reach. Primary IPv4/IPv6 subnets are maintained in the assignment sections above; only extra managed routes are managed here.',
  '目标网络 (CIDR)': 'Destination network (CIDR)',
  '例如 10.1.2.0/24 或 fd00:1::/64': 'e.g. 10.1.2.0/24 or fd00:1::/64',
  '下一跳地址': 'Next-hop address',
  '可选': 'Optional',
  '添加路由': 'Add Route',
  '目标 (CIDR)': 'Destination (CIDR)',
  '经由': 'Via',
  '尚未配置额外托管路由。': 'No extra managed routes configured yet.',
  '多播接收者限制': 'Multicast recipient limit',
  '启用广播': 'Enable broadcast',
  '账户信息': 'Account Info',
  '创建时间': 'Created at',
  '更新时间': 'Updated at',
  ...
  '这一步仅用于首次部署。若你已经创建过管理员，刷新后会自动进入完成步骤，不会重复重置数据库。': 'This step is only for the first deployment. If an administrator already exists, the wizard will skip to the finish step after refreshing without resetting the database.',
  ...
  '当前仅支持 SQLite 单实例部署。数据库文件将由程序使用默认路径管理。': 'Currently only single-instance SQLite deployment is supported. The database file will be managed by the program using the default path.',
  'MySQL 与 PostgreSQL 相关抽象暂时保留，但当前不在支持范围内。': 'MySQL and PostgreSQL abstractions are temporarily retained but are not currently supported.',
  '当前固定为 SQLite': 'Currently fixed to SQLite',

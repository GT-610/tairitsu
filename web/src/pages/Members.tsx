...
interface Member {
  id: string; // ZeroTier设备ID
  name: string; // 设备名称
  nodeId: string; // 设备ID（与id相同）
...
function Members() {
  ...
  const filteredMembers = members.filter(member => 
    ...
    member.nodeId.toLowerCase().includes(searchTerm.toLowerCase())
  );
  ...
  const handleSubmit = async () => {
    try {
      if (!networkId || !currentMember) {
        throw new Error('网络ID或成员ID不能为空');
      }
      
      // 更新现有成员（只允许更新设备名称和授权状态）
      const updatedData = {
              ...
              {filteredMembers.length === 0 ? (
                ...
                filteredMembers.map((member) => (
                  ...
                  >
                    ...
                    <TableCell>{member.nodeId}</TableCell>
                    <TableCell>
            ...
            <FormControlLabel
              control={
                <Switch
                  ...
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => setFormData({ ...formData, authorized: e.target.checked })}
                />
              }
              label={formData.authorized ? '已授权' : '未授权'}
            />
          </Box>
        </DialogContent>
        <DialogActions>

    ...
    <SettingsSectionCard title="IPv4分配" unsaved={unsaved}>
      <Box sx={{ mb: 3 }}>
        ...
        <Box sx={{ display: 'flex', gap: 2, alignItems: 'center', flexWrap: 'wrap' }}>
          <Button variant="outlined" onClick={onGenerateSubnetAndRange}>生成新的子网与范围</Button>
      ...
      <Paper variant="outlined" sx={{ p: 2.5, mb: 3, bgcolor: 'action.hover' }}>
        <Box sx={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 2, mb: draftValue.autoAssign ? 2 : 0, flexWrap: 'wrap' }}>
          <Box>
            <Typography variant="h6">自动分配 IPv4 地址给新成员设备</Typography>
            <Typography variant="body2" color="text.secondary">
          ...
          <>
            <Box sx={{ display: 'grid', gap: 2, gridTemplateColumns: { xs: '1fr', md: '1fr 1fr' } }}>
              <TextField
                ...
                helperText={draftValue.subnet ? `必须落在 ${draftValue.subnet} 内` : '请先填写 IPv4 子网'}
              />
              <TextField
                fullWidth
                label="结束 IPv4"
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 2, flexWrap: 'wrap', mt: 2 }}>
              <Box>
                <Typography variant="subtitle1">Active IPv4 Auto-Assign Range</Typography>
                <Typography variant="body2" color="text.secondary">使用 `Set Range` 将当前输入加入自动分配地址池列表。</Typography>
              </Box>
            ...
            {draftValue.pools.length > 0 ? (
              ...
              <Typography variant="body2" color="text.secondary" sx={{ mt: 2 }}>尚未配置自动分配地址池。</Typography>
            )}
          </>
        )}
      </Paper>
